...
namespace TaskManangerSystem.Controllers
{
    ...
    public class EmployeeController(ManagementSystemContext context) : ControllerBase
    {
        ...
        [HttpGet]
        [Authorize(Roles = "99")]
        public async Task<ActionResult<IEnumerable<BasePartInfo>>> GetEmployeeSystemAccounts()
            => await context.employees.Select(x => x.ToBasePartInfo()).ToListAsync();
        ...
        {
            if (employee.ExistsEncrypts(employeeSystemAccount.EmployeeAlias)) return "名称已存在";
        ...
        =>  employee.GetEncrypts(id)?.ToPartInfo();
        ...
        {
            EmployeeAccount? objs = employee.GetEmployee(id);
        ...
        {
            EmployeeAccount? employeeAccount = employee.GetEmployee(id);
